...
const ChatBot: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', content: "Hello! I am **Rakshak**, your safety companion. I've synced with your location to provide real-time assistance. How can I help you today?" }
  ]);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
          ...
          setUserLocation({ lat: latitude, lng: longitude, address: 'Location Active' });
        }
      }, (err) => {
    ...
    <div className="max-w-5xl mx-auto flex flex-col h-[82vh] bg-white rounded-[4rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-500">
      ...
      <div className="bg-slate-900 px-10 py-8 flex items-center justify-between text-white border-b border-white/5">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-black tracking-tight leading-none mb-2">Rakshak</h2>
            <div className="flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
              <span className="text-[10px] font-black text-white/50 uppercase tracking-[0.3em]">Network Active</span>
            </div>
          </div>
        </div>
        {userLocation && (
          <div className="hidden lg:flex items-center gap-4 px-6 py-3 bg-white/5 rounded-2xl border border-white/5 text-[11px] font-bold text-white/70 max-w-[320px]">
            ...
            <span className="truncate">{userLocation.address || "GPS Connected"}</span>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSend} className="p-10 bg-white border-t border-slate-100">
        <div className="flex gap-5">
          <input
            type="text"
            placeholder="Ask about nearby safe zones or safety protocols..."
            value={input}
          ...
          <button 
            ...
            className="w-20 h-20 bg-slate-900 text-white rounded-[2rem] shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center disabled:opacity-20 active:scale-90 group"
          >
            <i className={`fas ${isTyping ? 'fa-circle-notch fa-spin' : 'fa-paper-plane-top'} text-2xl transition-transform`}></i>
          </button>
        </div>
